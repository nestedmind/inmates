...
- Track coordination state outside your own context. A status file with one entry per stream (ticket, PR number, review round, what it waits on) survives context compaction. Update it when state changes.
...
- Set `user.name` and `user.email` in that persona's worktree only, and leave the global git config alone.
