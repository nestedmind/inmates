...
- Read `models:` from `.larceny/config.md` before dispatching any persona. `models: default` (or the key missing) means the shipped frontmatter's model is already right — pass no override. `models: harness-default` means pass no explicit `model` at spawn time for any persona, so the harness's own default applies instead of the shipped frontmatter. Either mode can carry indented `<persona name>: <model>` override lines; for a persona named there, pass that model explicitly at spawn time (the Agent tool's `model` parameter), which takes precedence over the shipped `agents/*.md` frontmatter. A persona with no override line keeps the baseline for that mode.
