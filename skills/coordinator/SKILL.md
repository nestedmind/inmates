...
- Read `models:` from `.larceny/config.md` before dispatching any coder, following "Model overrides" in `docs/agent-lifecycle.md`. The same rule applies to every other persona spawn (Sara, Linc, Tbag, a renamed coordinator) through their own spawn commands and the auto-spawn path — this bullet is the coder-dispatch instance of that one shared rule, not a separate one.
