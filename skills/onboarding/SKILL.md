...
7. **Ask about Tbag, Linc and Sara, one yes/no question.** "Want to rename the reviewer, advisor or teacher persona, or keep the default cast (Tbag, Linc, Sara)?" Default answer is keep-as-is. This is independent of the roster question in step 6: it can be skipped even when the roster was customized, and vice versa. Ask it before the model question in step 8, so a model override recorded there uses each persona's final name, not a shipped default that gets renamed a step later.
...
| "The roster question in step 6 already covers Tbag, Linc and Sara." | It does not. That question renames the coordinator and coders only. Renaming the reviewer, advisor or teacher is a separate question, step 7. |
