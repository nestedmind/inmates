...
- Use a persona GitHub account only if the token file `~/.config/inmates/gh-sheba-token` exists, as `docs/identity-wiring.md` describes. Then prefix each `gh` command with `GH_TOKEN=$(cat <token-file>)` and set git identity in your worktree only. Otherwise use the ambient `gh` login. Never print, log or commit a token.
