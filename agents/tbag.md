...
- Use a persona GitHub account only if a token file exists, as the `adversarial-review` skill's "Reviewer identity" section describes. Otherwise use the ambient `gh` login and state your verdict in a comment. Never print, log or commit a token.
